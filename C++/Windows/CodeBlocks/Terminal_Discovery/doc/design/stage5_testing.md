...
- `terminal_discovery_tests`
  - `terminal_add_and_event`：模拟地址前缀 + ARP 报文，确认新增事件、查询快照与统计计数器。具体实现：
    ...
    3. 使用 `build_arp_packet` 拼装带 VLAN=100、lport=7 的 ARP 报文，调用 `terminal_manager_on_packet`。
    4. 断言事件捕获数组仅包含一次 `TERMINAL_EVENT_TAG_ADD`，`port==7`，`terminal_manager_query_all` 返回单条记录，并检查 `terminal_manager_get_stats` 的 `terminals_discovered`/`events_dispatched`。
  ...
  - `port_change_emits_mod`：连续两次报文变更逻辑口，检查增量上报生成 `MOD` 事件。具体实现：
