...
- 外部 demo 在完成桥接模块初始化后调用 `td_switch_mac_demo_dump()`，即可打印桥接接口返回的 MAC/VLAN/ifindex 信息；函数会先调用 `td_switch_mac_get_capacity()` 估算缓存需求，再一次性分配缓冲区并触发 `td_switch_mac_snapshot()`，常规输出写入标准输出，错误信息写入标准错误；由于底层 `getDevUcMacAddress` 不检查缓冲区大小，请务必保证桥接层返回的容量与实际条目数一致，并在桥接内部复用一块 `SwUcMacEntry` 静态缓冲区与 SDK 交互；
