...
- 头文件分层：`src/include/` 仅保留平台无关公共头；平台专属头必须移出公共 include 并放入对应平台目录，由平台 makefile 定向 `-I`。Realtek 相关头（如 `td_switch_mac_bridge.h`）放入 Realtek 路径并仅在 Realtek 构建包含；netforward sidecar 头（如 `netforward_sidecar.h`）放入 `src/sidecar/` 并仅在 netforward 构建包含。公共静态库导出的对外头文件必须保持平台无关，平台特有接口不得进入公共 include。
