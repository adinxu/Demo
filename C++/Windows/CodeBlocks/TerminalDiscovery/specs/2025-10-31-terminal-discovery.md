...
9. **守护进程集成**：
   ...
   - 初始化期间需暴露回调注入点，支持宿主进程在启动流程中通过参数传入并调用 `terminal_manager_set_event_sink` 注册平台侧的增量上报处理函数；不同平台可传入各自的回调实现，未传入时需拒绝启动。
...
- **初始化与嵌入**：
   ...
   4. 在 `terminal_manager_start` 前提供回调注入点，初始化参数中必须包含 `terminal_manager_set_event_sink` 需要的回调函数指针；函数内部完成注册，并在未提供回调时返回错误。
