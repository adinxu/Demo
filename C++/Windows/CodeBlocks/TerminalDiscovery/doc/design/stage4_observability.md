...
- 主程序新增 `terminal_stats` 组件日志：支持 `kill -USR1 <pid>` 立即打印一次 `terminal_manager_get_stats` 快照，并提供交互式命令行（提示符 `td>`）；运行期间可以输入 `stats` 或 `dump terminal|prefix|binding|mac queue|mac state` 等命令即时获取运行快照，方便在线排障。
