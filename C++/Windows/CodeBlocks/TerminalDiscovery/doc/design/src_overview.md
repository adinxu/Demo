...
- 主循环结合 `handle_stats_signal` 与交互式命令行监听处理：接收 `stats`、`dump terminal/prefix/binding/mac queue/mac state` 等指令，即时调用 `log_manager_stats` 或 `td_debug_dump_*` 输出当前快照；输入 `help` 可查看命令列表，提示符 `td>` 表示可继续输入。
