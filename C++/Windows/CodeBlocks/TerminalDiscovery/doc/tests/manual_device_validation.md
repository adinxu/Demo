...
| H02 | 交互式监控 | 输入 `stats`、`show config`、`set keepalive 300`、`ignore-vlan add 200`、`ignore-vlan remove 200`、`ignore-vlan clear`、`exit` 等 | 长时间运行 | 按需在命令行打印统计/配置，`show config` 输出 `terminal_config` 组件日志（含 keepalive/miss/holdoff/scan/max 与 `ignored_vlans=[...]`），`dump *` 命令输出具体表项，`set` 命令可调整运行时参数，`ignore-vlan` 命令可在线维护忽略 VLAN 列表，`exit|quit` 命令可安全终止守护进程 |
